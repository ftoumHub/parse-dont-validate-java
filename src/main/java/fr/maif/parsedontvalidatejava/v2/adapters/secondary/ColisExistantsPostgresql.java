...
public class ColisExistantsPostgresql implements ColisExistants {
    ...
    public Mono<Colis.ColisExistant> mettreAJourColis(Colis.ColisExistant colisExistant) {
        println("==> mettreAJourColis");
        var jsonString = fr.maif.json.Json.stringify(
                fr.maif.json.Json.toJson(ColisDatabase.fromColis(colisExistant), JsonWrite.auto())
        );
        println(Json.of(jsonString).toString());
        return client.sql("""
                       update colis
                          set ("reference", 
                               "type",
